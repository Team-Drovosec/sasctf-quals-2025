...
pm = 9
n = len({})
pcomp_invocation = "python ./compress.py"  # information for zpaqd about preprocessor invocation,
...
def pcomp(c):
    global pstate, key_pos, i_rc4, j_rc4
    ...
    if pstate == 0:
        pM[KEY_BUF + key_pos] = c
        key_pos += 1
        if key_pos < 16:
            return

        if (((pM[KEY_BUF + 9] | pM[KEY_BUF + 0]) ^ (pM[KEY_BUF + 12] ^ (pM[KEY_BUF + 15] ^ pM[KEY_BUF + 1])))) & 0xFF != 196:
            pstate=2
        if ((pM[KEY_BUF + 8] & (pM[KEY_BUF + 14] + pM[KEY_BUF + 15]))) & 0xFF != 0:
            pstate=2
        if (((pM[KEY_BUF + 11] + pM[KEY_BUF + 9]) | ((pM[KEY_BUF + 6] + pM[KEY_BUF + 11]) + (pM[KEY_BUF + 3] | pM[KEY_BUF + 13])))) & 0xFF != 50:
            pstate=2
        if (((pM[KEY_BUF + 10] | pM[KEY_BUF + 4]) | ((pM[KEY_BUF + 7] | pM[KEY_BUF + 3]) & (pM[KEY_BUF + 12] | pM[KEY_BUF + 11])))) & 0xFF != 238:
            pstate=2
        if ((pM[KEY_BUF + 6] ^ (pM[KEY_BUF + 9] | pM[KEY_BUF + 8]))) & 0xFF != 56:
            pstate=2
        if ((pM[KEY_BUF + 10] ^ (pM[KEY_BUF + 14] ^ pM[KEY_BUF + 3]))) & 0xFF != 88:
            pstate=2
        if (((pM[KEY_BUF + 12] ^ pM[KEY_BUF + 1]) + (pM[KEY_BUF + 13] | (pM[KEY_BUF + 15] ^ pM[KEY_BUF + 1])))) & 0xFF != 69:
            pstate=2
        if (((pM[KEY_BUF + 2] | pM[KEY_BUF + 9]) ^ ((pM[KEY_BUF + 0] | pM[KEY_BUF + 14]) | (pM[KEY_BUF + 8] & pM[KEY_BUF + 11])))) & 0xFF != 69:
            pstate=2
        if (((pM[KEY_BUF + 0] & pM[KEY_BUF + 7]) + ((pM[KEY_BUF + 9] | pM[KEY_BUF + 8]) ^ (pM[KEY_BUF + 0] + pM[KEY_BUF + 8])))) & 0xFF != 24:
            pstate=2
        if ((pM[KEY_BUF + 12] | (pM[KEY_BUF + 14] | pM[KEY_BUF + 15]))) & 0xFF != 255:
            pstate=2
        if (((pM[KEY_BUF + 6] & pM[KEY_BUF + 4]) | (pM[KEY_BUF + 4] ^ (pM[KEY_BUF + 5] & pM[KEY_BUF + 11])))) & 0xFF != 230:
            pstate=2
        if (((pM[KEY_BUF + 13] + pM[KEY_BUF + 4]) + (pM[KEY_BUF + 0] + pM[KEY_BUF + 3]))) & 0xFF != 15:
            pstate=2
        if ((pM[KEY_BUF + 13] & pM[KEY_BUF + 15])) & 0xFF != 4:
            pstate=2
        if ((pM[KEY_BUF + 7] ^ pM[KEY_BUF + 4])) & 0xFF != 214:
            pstate=2
        if ((pM[KEY_BUF + 4] & pM[KEY_BUF + 12])) & 0xFF != 6:
            pstate=2
        if ((pM[KEY_BUF + 6] + pM[KEY_BUF + 8])) & 0xFF != 36:
            pstate=2
        if ((pM[KEY_BUF + 12] & pM[KEY_BUF + 10])) & 0xFF != 6:
            pstate=2
        if ((pM[KEY_BUF + 10] ^ pM[KEY_BUF + 15])) & 0xFF != 144:
            pstate=2
        if (((pM[KEY_BUF + 7] ^ pM[KEY_BUF + 9]) | (pM[KEY_BUF + 3] & (pM[KEY_BUF + 10] & pM[KEY_BUF + 11])))) & 0xFF != 226:
            pstate=2
        if (((pM[KEY_BUF + 7] & pM[KEY_BUF + 3]) + ((pM[KEY_BUF + 1] & pM[KEY_BUF + 1]) & (pM[KEY_BUF + 5] | pM[KEY_BUF + 2])))) & 0xFF != 9:
            pstate=2
        if (((pM[KEY_BUF + 5] + pM[KEY_BUF + 12]) + ((pM[KEY_BUF + 3] ^ pM[KEY_BUF + 15]) + (pM[KEY_BUF + 11] & pM[KEY_BUF + 1])))) & 0xFF != 203:
            pstate=2
        if ((pM[KEY_BUF + 9] & (pM[KEY_BUF + 13] + pM[KEY_BUF + 0]))) & 0xFF != 26:
            pstate=2
        if (((pM[KEY_BUF + 11] + pM[KEY_BUF + 13]) | ((pM[KEY_BUF + 4] + pM[KEY_BUF + 6]) + (pM[KEY_BUF + 12] + pM[KEY_BUF + 3])))) & 0xFF != 191:
            pstate=2
        if (((pM[KEY_BUF + 11] + pM[KEY_BUF + 8]) + (pM[KEY_BUF + 3] ^ pM[KEY_BUF + 15]))) & 0xFF != 221:
            pstate=2
        if ((pM[KEY_BUF + 8] + (pM[KEY_BUF + 2] + pM[KEY_BUF + 5]))) & 0xFF != 234:
            pstate=2
        if (((pM[KEY_BUF + 3] & pM[KEY_BUF + 1]) | (pM[KEY_BUF + 10] | pM[KEY_BUF + 2]))) & 0xFF != 167:
            pstate=2
        if ((pM[KEY_BUF + 3] | (pM[KEY_BUF + 3] & pM[KEY_BUF + 7]))) & 0xFF != 34:
            pstate=2
        if ((pM[KEY_BUF + 3] + (pM[KEY_BUF + 7] | pM[KEY_BUF + 0]))) & 0xFF != 252:
            pstate=2
        if (((pM[KEY_BUF + 8] & pM[KEY_BUF + 3]) ^ ((pM[KEY_BUF + 10] ^ pM[KEY_BUF + 8]) | (pM[KEY_BUF + 5] | pM[KEY_BUF + 1])))) & 0xFF != 207:
            pstate=2
        if (((pM[KEY_BUF + 6] | pM[KEY_BUF + 13]) ^ ((pM[KEY_BUF + 14] & pM[KEY_BUF + 12]) & (pM[KEY_BUF + 4] + pM[KEY_BUF + 6])))) & 0xFF != 135:
            pstate=2
        if (((pM[KEY_BUF + 2] | pM[KEY_BUF + 12]) + ((pM[KEY_BUF + 12] & pM[KEY_BUF + 13]) & (pM[KEY_BUF + 8] | pM[KEY_BUF + 4])))) & 0xFF != 172:
            pstate=2
        if ((pM[KEY_BUF + 1] & (pM[KEY_BUF + 9] + pM[KEY_BUF + 13]))) & 0xFF != 15:
            pstate=2
        if (((pM[KEY_BUF + 14] | pM[KEY_BUF + 9]) & (pM[KEY_BUF + 5] | pM[KEY_BUF + 14]))) & 0xFF != 252:
            pstate=2
        if ((pM[KEY_BUF + 0] + (pM[KEY_BUF + 8] | pM[KEY_BUF + 13]))) & 0xFF != 127:
            pstate=2
        if ((pM[KEY_BUF + 12] ^ (pM[KEY_BUF + 9] ^ pM[KEY_BUF + 11]))) & 0xFF != 149:
            pstate=2
        if (((pM[KEY_BUF + 10] ^ pM[KEY_BUF + 0]) + ((pM[KEY_BUF + 8] | pM[KEY_BUF + 12]) | (pM[KEY_BUF + 2] & pM[KEY_BUF + 3])))) & 0xFF != 163:
            pstate=2
        ...
        i = 0
        while i < 256:
            j_rc4 = (j_rc4 + pM[i] + pM[KEY_BUF + (i & 15)]) & 0xFF
            tmp        = pM[i]
            pM[i]      = pM[j_rc4]
            pM[j_rc4]  = tmp
            i += 1
        ...
        i_rc4 = (i_rc4 + 1) & 0xFF
        j_rc4 = (j_rc4 + pM[i_rc4]) & 0xFF
        tmp        = pM[i_rc4]
        pM[i_rc4]  = pM[j_rc4]
        pM[j_rc4]  = tmp
        k          = pM[(pM[i_rc4] + pM[j_rc4]) & 0xFF]
        out(c ^ k)
